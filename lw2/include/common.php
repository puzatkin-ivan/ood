<?php

require_once __DIR__ . '/../src/calculator/StatsCalculator.php';
require_once __DIR__ . '/../src/calculator/WindDirectionStatsCalculator.php';
require_once __DIR__ . '/../src/sensors/Sensors.php';
require_once __DIR__ . '/../src/observer/ObserverInterface.php';
require_once __DIR__ . '/../src/observer/Entity.php';
require_once __DIR__ . '/../src/observer/Follower.php';
require_once __DIR__ . '/../src/observer/StatsDisplay.php';
require_once __DIR__ . '/../src/observer/Display.php';
require_once __DIR__ . '/../src/observer/mock/ObserverMock.php';
require_once __DIR__ . '/../src/observer/mock/DistinguishingSensorsObserverMock.php';
require_once __DIR__ . '/../src/observable/ObservableInterface.php';
require_once __DIR__ . '/../src/observable/Observable.php';
require_once __DIR__ . '/../src/observable/WeatherData.php';
require_once __DIR__ . '/../src/WeatherStation.php';
require_once __DIR__ . '/../vendor/autoload.php';